...
<title id="Timerhead">Profaktura за префърцунените - Land of Roses doo: baza podataka</title>
...
else $sql="SELECT * FROM prodaja ORDER BY `ID` DESC LIMIT 1";
...
$result=mysqli_query($mysqli,$sql) or die ('1: '.$sql.'<br/>'.mysql_error());
$row=$result->fetch_assoc();
...
$danas=date('d.m.Y.');
$desnibox='<b>Račun-Otpremnica broj: ';
if (empty($brracuna)) $desnibox.=$brpracuna;
else $desnibox.=$brracuna;
$desnibox.='</b><br/><br/>Rok plaćanja: '.$rok.'<br/>Datum prometa: '.$datprometa.'<br/><br/><b>TR: '.$brracunau.'<br/>Poziv: '.$pozivnb.'</b>';
...
while ($row=$result->fetch_assoc()) {
	...
	$sve.='<tr><td style="text-align:center">'.$iduprodaji.'</td><td style="text-align:center">'.$proizvod.'</td><td style="text-align:left;max-width:300px">'.$naziv.'</td><td style="text-align:center">'.$kolicina.'</td><td style="text-align:center">kom</td><td>'.$pcena.' RSD</td><td>'.$vpc.' RSD</td><td>'.$rabat.' %</td><td>'.$osnovica.' RSD</td><td>'.$ppdv.' RSD</td><td>'.$ppcena.' RSD</td></tr>';
}
$sve.='</table>';
}
else {
	...
	foreach($redovi as $xx => $yy) {
		...
		while ($row=$result->fetch_assoc()) {
			...
			$ppdv=number_format($ppdv, 2, ',', '.');
			$ppcena=number_format($ppcena, 2, ',', '.');
			$sve[$stranicabr].='<tr><td style="text-align:center">'.$iduprodaji.'</td><td style="text-align:center">'.$proizvod.'</td><td style="text-align:left;max-width:300px">'.$naziv.'</td><td style="text-align:center">'.$kolicina.'</td><td style="text-align:center">kom</td><td>'.$pcena.' RSD</td><td>'.$vpc.' RSD</td><td>'.$rabat.' %</td><td>'.$osnovica.' RSD</td><td>'.$ppdv.' RSD</td><td>'.$ppcena.' RSD</td></tr>';
		}
	...
	$desnibox='<b>Račun otpremnica: '.$brpracuna.'</b><br/><br/><b>Rok plaćanja: '.$rok.'</b><br/>Datum prometa: '.$datprometa.'<br/><br/><b>TR: '.$brracunau.'</b><br/><b>Poziv: '.$pozivnb.'</b>';
	$dolbox=$bezpopusta.' RSD<br/>'.$popust.' RSD<br/>'.$bezpdva.' RSD<br/>'.$iznospdv.' RSD<br/>'.$zauplatu.' RSD';
