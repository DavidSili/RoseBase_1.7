...
$sve='';
if ($brstranica==1) {
$sve.='<table style="font-size:12" border="1"><tr style="background:#ddd"><th stlye="width:16px">Br.</th><th style="width:50px">Šifra proizvoda</th><th>Vrsta robe - usluga</th><th style="width:20px">Kol.</th><th>JM</th><th style="width:68px">MPC</th><th style="width:40px">R %</th><th style="width:74px">Osnovica</th><th style="width:62px">PDV</th><th style="width:74px">Vred. sa PDV</th></tr>';
...
while ($row=$result->fetch_assoc()) {
	...
	$osnovica=$kolicina*$mpbezpdv*(100-$rabat)/100;
	...
	$sve.='<tr><td style="text-align:center">'.$redbroj.'</td><td style="text-align:center">'.$proizvod.'</td><td style="text-align:left;max-width:300px">'.$naziv.'</td><td style="text-align:center">'.$kolicina.'</td><td style="text-align:center">kom</td><td>'.$pcena.' RSD</td><td>'.$rabat.' %</td><td>'.$osnovica.' RSD</td><td>'.$ppdv.' RSD</td><td>'.$ppcena.' RSD</td></tr>';
	$redbroj++;
...
$sve.='</table>';
	...
	foreach($redovi as $xx => $yy) {
		...
		while ($row=$result->fetch_assoc()) {
			...
			$ppdv=number_format($ppdv, 2, ',', '.');
			$ppcena=number_format($ppcena, 2, ',', '.');
			$sve[$stranicabr].='<tr><td style="text-align:center">'.$redbroj.'</td><td style="text-align:center">'.$proizvod.'</td><td style="text-align:left;max-width:300px">'.$naziv.'</td><td style="text-align:center">'.$kolicina.'</td><td style="text-align:center">kom</td><td>'.$pcena.' RSD</td><td>'.$rabat.' %</td><td>'.$osnovica.' RSD</td><td>'.$ppdv.' RSD</td><td>'.$ppcena.' RSD</td></tr>';
			$redbroj++;
			$bezpopusta=$bezpopusta+($cpcena*$kolicina);
			$bezpdva=$bezpdva+$cosnovica;
			$iznospdv=$iznospdv+$cppdv;
...
foreach($redovi as $xx => $yy) {
	...
	<div style="overflow:hidden">
		...
		<div style="float:right;text-align:right">LAND OF ROSES DOO Pančevo,<br/>26000, ul. Karađorđeva 30<br/>Matični broj: 20945508<br/>PIB: 108173517<br/>Tel: +381 60 8080 613<br/>br. računa: 180-1281210041143-63<br/>web: www.bugarska-ruza.rs <br/>e-mai: office@bugarska-ruza.rs</div>
	</div>
