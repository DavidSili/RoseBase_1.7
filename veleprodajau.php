...
if(isset($_POST) && !empty($_POST)) {
	...
	if (isset($_POST['del'])) {
		...
		foreach($proizvodi as $oo) {
			...
			$sql1b='SELECT kolicina FROM zalihe WHERE skladiste="'.$skladiste.'" AND proizvod="'.$oo.'"';
...
<title id="Timerhead">Internet prodaja - Land of Roses doo: baza podataka</title>
...
$brendxx="";
$sql="SELECT `prodaja`.`ID` ID,`prodaja`.`brpracuna` brpracuna,`prodaja`.`brracuna` brracuna,`skladista`.`naziv` nskladista, `prodaja`.`skladiste` skladiste FROM prodaja LEFT JOIN skladista ON prodaja.skladiste = skladista.ID WHERE `prodaja`.`kupac` <> '84' ORDER BY `prodaja`.`ID` DESC";
$result=mysql_query($sql)or die;
while($row=mysql_fetch_assoc($result)) {
	...
	if ($nskladista=="Pančevo") $bgn=' style="background:#ccf"';
	elseif ($nskladista=="Biofresh") $bgn=' style="background:#eee"';
	else $bgn='';
	if (empty($brracuna)) echo '<a href="#" onclick="izmena('.$ID.','.$skladiste.')"><i><b>'.$ID.' - '.$brpracuna.'</b></i></a>&nbsp&nbsp&nbsp<span'.$bgn.'>'.$nskladista.'</span><br/>';
...
<div style="width:325px;top:427px;left:205px;position:absolute;bottom:0;background:#fff;opacity:0.5">
</div>
...
echo $ID;
	...
	<div class="iur">
		<div class="iul">Prodavac</div>
		<select id="yskladiste" type="text" name="skladiste" class="iud" style="width:153px" onchange="proizvodi()" >
<?php
...
<?php
$sql='SELECT partneri.ID ID, partneri.ime ime, partneri.prezime prezime, partneri.firma firma, gpartnera.ID posao FROM partneri LEFT JOIN gpartnera ON partneri.gpartnera = gpartnera.ID ORDER BY partneri.prezime ASC, partneri.ime ASC';
$result=mysql_query($sql)or die;
while($row=mysql_fetch_assoc($result)) {
	$ID=$row['ID'];
	$ime=$row['ime'];
...
<?php
$godina=date('Y');
$sql='SELECT brpracuna FROM prodaja WHERE skladiste="2" ORDER BY brpracuna DESC LIMIT 1';
$result=mysql_query($sql)or die;
...
while($row=mysql_fetch_assoc($result)) {
	...
	if ($sgodina!=$godina) {
	$npracunLoR=$godina.'-00001';
	}
	else {
	$brpracuna=substr($brpracuna, 5);
	$nbroj=str_pad(($brpracuna+1), 5, "0", STR_PAD_LEFT);
	$npracunLoR=$godina.'-'.$nbroj;
	}
}}
else {
$npracunLoR=$godina.'-00001';
}
$sql='SELECT brpracuna FROM prodaja WHERE skladiste="22" ORDER BY brpracuna DESC LIMIT 1';
$result=mysql_query($sql)or die;
if (mysql_num_rows($result)>0) {
while($row=mysql_fetch_assoc($result)) {
	$brpracuna=$row['brpracuna'];
	$sgodina=substr($brpracuna, 0, -6);
	if ($sgodina!=$godina) {
	$npracunBF=$godina.'-00001';
	}
	else {
	$brpracuna=substr($brpracuna, 5);
	$nbroj=str_pad(($brpracuna+1), 5, "0", STR_PAD_LEFT);
	$npracunBF=$godina.'-'.$nbroj;
	}
}}
else {
$npracunBF=$godina.'-00001';
...
$sql='SELECT brracuna FROM prodaja WHERE skladiste="2" ORDER BY brracuna DESC LIMIT 1';
$result=mysql_query($sql)or die;
if (mysql_num_rows($result)>0) {
while($row=mysql_fetch_assoc($result)) {
	$brracuna=$row['brracuna'];
	...
	if ($sgodina!=$godina) {
	$nracunLoR=$godina.'-00001';
	}
	else {
	$brracuna=substr($brracuna, 5);
	$nbroj=str_pad(($brracuna+1), 5, "0", STR_PAD_LEFT);
	$nracunLoR=$godina.'-'.$nbroj;
	}
}}
else {
$nracunLoR=$godina.'-00001';
}
$sql='SELECT brracuna FROM prodaja WHERE skladiste="22" ORDER BY brracuna DESC LIMIT 1';
$result=mysql_query($sql)or die;
if (mysql_num_rows($result)>0) {
while($row=mysql_fetch_assoc($result)) {
	$brracuna=$row['brracuna'];
	$sgodina=substr($brracuna, 0, -6);
	if ($sgodina!=$godina) {
	$nracunBF=$godina.'-00001';
	}
	else {
	$brracuna=substr($brracuna, 5);
	$nbroj=str_pad(($brracuna+1), 5, "0", STR_PAD_LEFT);
	$nracunBF=$godina.'-'.$nbroj;
	}
}}
else {
$nracunBF=$godina.'-00001';
...
?>
	<div class="iur">
		...
		<input type="hidden" id="pracunLoR" value="<?php echo $npracunLoR; ?>" />
		<input type="hidden" id="racunLoR" value="<?php echo $nracunLoR; ?>" />
		<input type="hidden" id="pracunBF" value="<?php echo $npracunBF; ?>" />
		<input type="hidden" id="racunBF" value="<?php echo $nracunBF; ?>" />
	...
	<div class="iur">
		...
		<input id="ydatprometa" type="date" name="datprometa" class="iud" />
		<div style="clear:both;"></div>
	</div>
	<div class="iur">
...
function predprodaj()
	{
		var pracunLoR = document.getElementById("pracunLoR").value;
		var pracunBF = document.getElementById("pracunBF").value;
		var skladiste = document.getElementById("yskladiste").value;
		if (skladiste=="2") {
		document.getElementById("ybrpracuna").value=pracunLoR;
		document.getElementById("ypozivnb").value=pracunLoR;
		}
		else if (skladiste=="22") {
		document.getElementById("ybrpracuna").value=pracunBF;
		document.getElementById("ypozivnb").value=pracunBF;
		}
		else {
		document.getElementById("ybrpracuna").value="";
		document.getElementById("ypozivnb").value="";
		}
	}
function prodaj()
	{
		var racunLoR = document.getElementById("racunLoR").value;
		var racunBF = document.getElementById("racunBF").value;
		var skladiste = document.getElementById("yskladiste").value;
		if (skladiste=="2") document.getElementById("ybrracuna").value=racunLoR;
		else if (skladiste=="22") document.getElementById("ybrracuna").value=racunBF;
		else document.getElementById("ybrracuna").value="";
	}
function izmena(posebno,skladiste)
	{
		...
		$.getJSON('ajax/veleprodajaui.php', {posebno: posebno, skladiste: skladiste, osoba: osoba}, function(data) {
			$('#yid').val(data.yid);
		...
		$.getJSON('ajax/veleprodajaup.php', {posebno: posebno, skladiste: skladiste}, function(data) {
...
function proizvodi(xx)
	{
		var skladiste = document.getElementById("yskladiste").value;
		var osoba= document.getElementById("ykupac").value;
		$.getJSON('ajax/veleprodajaup.php', {posebno: xx, skladiste: skladiste, osoba: osoba}, function(data) {
			$('#trecakolona').html(data.ysortostali);
			$('#desnakolona').html(data.ysorttu);
